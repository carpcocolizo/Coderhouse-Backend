...
function submitHandler(event) {
    ...
    const productInfo = { title: titleForm.value, price: priceForm.value, thumbnail: thumbnailForm.value }
...
function submitHandlerMessage(event) {
    event.preventDefault()
    const messageInfo = { email: emailInput.value, date: new Date(),  message: messageInput.value }
...
socket.on('server:message', messageInfo => {
    if (messageInfo == null) {
        ...
        renderMessages(messageInfo.flat())
    }
})
